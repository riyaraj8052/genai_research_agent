...
from A01_prompts import lead_researcher_prompt
from A06_research_agent import researcher_agent
from A07_supervisor_agent_schema import (
    SupervisorState, 
    ConductResearch, 
    ResearchComplete
)
from A04_utils import model_config, get_today_str, think_tool
...
supervisor_model = ChatDatabricks(endpoint=LLM_ENDPOINT_NAME)
...
def supervisor_tools(state: SupervisorState) -> Command[Literal["supervisor", "__end__"]]:
    ...
    - Launching multiple research agents for different topics
    ...
    if exceeded_iterations or no_tool_calls or research_complete:
        ...
        try:
            ...
            # Handle ConductResearch calls (synchronous)
            if conduct_research_calls:
                # Launch parallel research agents
                for tool_call in conduct_research_calls:
                    observation = researcher_agent.invoke({
                            "researcher_messages": [
...
supervisor_agent_builder = StateGraph(SupervisorState)
supervisor_agent_builder.add_node("supervisor", supervisor)
supervisor_agent_builder.add_node("supervisor_tools", supervisor_tools)
supervisor_agent_builder.add_edge(START, "supervisor")
supervisor_agent = supervisor_agent_builder.compile()
